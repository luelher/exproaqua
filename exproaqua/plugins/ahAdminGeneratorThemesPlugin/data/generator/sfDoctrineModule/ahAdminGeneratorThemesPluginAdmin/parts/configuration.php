...
<?php unset($this->config['form']['class']) ?>
   ...
   * @return sfForm
   */
  public function getNewForm($object = null, $options = array())
  {
    $class = $this->getNewFormClass();
    
    return new $class($object, array_merge($this->getFormOptions(), $options));
  }
  
  /**
   * Gets the new form class name.
   *
   * @return string The new form class name
   */
  public function getNewFormClass()
  {
    return '<?php echo isset($this->config['new']['class']) ? $this->config['new']['class'] : $this->getModelClass().'Form' ?>';
<?php unset($this->config['new']['class']) ?>
  }
  
  /**
   * Gets a new edit form object.
   *
   * @param  mixed $object
   * @param  array $options
   *
   * @return sfForm
   */
  public function getEditForm($object = null, $options = array())
  {
    $class = $this->getEditFormClass();

    return new $class($object, array_merge($this->getFormOptions(), $options));
  }
  
  /**
   * Gets the edit form class name.
   *
   * @return string The edit form class name
   */
  public function getEditFormClass()
  {
    return '<?php echo isset($this->config['edit']['class']) ? $this->config['edit']['class'] : $this->getModelClass().'Form' ?>';
<?php unset($this->config['edit']['class']) ?>
  }
